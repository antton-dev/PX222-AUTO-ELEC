...
\subsection{Passage de 40 à 60V}

\subsubsection{Analyse du changement}
Initialement, la tension d'alimentation avait été fixée à $E_0 = 40\text{ V}$ en raison du manque d'une grille de protection. Lors du passage à la tension maximale de $E_0 = 60\text{ V}$ les relevés expérimentaux ont mis en évidence une augmentation significative de l'erreur statique en régime permanent, dépassant les $10\text{ mV}$ observés précédemment.

En conservant le réglage issu de la simulation ($\omega_i = 36,95\text{ rad/s}$), l'observation de la mesure du courant a révélé une \textbf{erreur statique ($\varepsilon$) trop importante} en régime permanent. L'intégrateur était trop lent pour compenser efficacement cette erreur. Il a donc été décidé de modifier $\omega_i$ afin de donner plus de poids à l'action intégrale et d'annuler cette erreur statique.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{img/analog_60V.png}
    \end{figure}

\clearpage

\subsubsection{Calcul de la nouvelle pulsation d'intégration $\omega_i$}
...
Le calcul de la nouvelle pulsation d'intégration réelle $\omega_{i}$ donne alors :
\begin{align}
    \omega_{i} &= \frac{1}{390 \times 10^{3} \times 100 \times 10^{-9}} \\
    \omega_{i} &= \frac{1}{0,039} \approx 25,64\text{ rad/s}
\end{align}

Nous retenons pour la suite la valeur expérimentale validée de $\omega_{i} \approx 25,6\text{ rad/s}$. Voici le nouveau graphique :

\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{img/analog_60V_v2.png}
    \end{figure}

\subsubsection{Analyse des résultats expérimentaux et performances}
L'introduction de cette nouvelle configuration ($\omega_i \approx 25,6\text{ rad/s}$ et $R_{212} = 390\text{ k}\Omega$) sous une alimentation de $60\text{ V}$ a permis d'obtenir le comportemnt attendu.

Le décodage des curseurs de mesure permet de valider les performances temporelles suivantes du régime transitoire :
\begin{itemize}
    \item \textbf{Constante de temps $\tau$} : Mesurée graphiquement à $63\%$ de la valeur finale du courant de l'échelon, nous relevons :
    \begin{equation*}
        \tau = 37\text{ ms}
    \end{equation*}
    Le système est nettement plus rapide que lors des premiers essais physiques où $\tau$ avoisinait les $50\text{ ms}$.
    
    \item \textbf{Temps de réponse à 95\% ($t_{r95\%}$)} : Le courant rejoint et s'établit durablement dans la bande des $\pm 5\%$ autour de sa valeur de consigne au bout de :
    \begin{equation*}
        t_{r95\%} = 80\text{ ms}
    \end{equation*}
    Ce résultat marque une nette amélioration par rapport aux $130\text{ ms}$ mesurés précédemment.
    
    \item \textbf{Précision et Stabilité} : L'erreur statique en régime permanent est désormais parfaitement compensée, le signal de mesure convergeant idéalement vers la consigne sans oscillation parasite ni dépassement critique.
\end{itemize}

L'augmentation de la tension à $60\text{ V}$ combinée à l'abaissement de la pulsation d'intégration à $25,6\text{ rad/s}$ offre le meilleur compromis performance/robustesse pour notre asservissement de courant.

\subsection{Bilan de l'implémentation du correcteur numérique et limites de l'approche}

\subsubsection{Analyse des difficultés et constat d'échec}
...
Cependant, nous ne sommes pas parvenus à faire fonctionner l'asservissement en boucle fermée par cette méthode numérique. Lors des tests physiques sur la maquette, le système a présenté une instabilité critique et n'a pas été capable de réguler le courant autour de sa consigne. Plusieurs facteurs interdépendants expliquent ce comportement :
\begin{itemize}
    \item \textbf{Gigue temporelle (Jitter)} : L'utilisation de la fonction \texttt{HAL\_Delay()} au sein de la boucle principale détruit la déterminisme temporel indispensable à un calcul d'intégration discret ($I_k = I_{k-1} + \varepsilon_k \cdot T_s$). La période d'échantillonnage $T_s$ réelle a subi de fortes fluctuations.
    \item \textbf{Conflits de pilotes matériels} : L'activation de l'ADC1 via la bibliothèque HAL a provoqué des conflits d'allocation de registres, générant des plantages de type \textit{HardFault} au démarrage de la carte. Bien qu'un contournement par accès direct aux registres (CMSIS) ait permis de restaurer la stabilité matérielle et de lire la broche PA5, le temps de conversion cumulé à la scrutation a bridé la réactivité de la boucle.
\end{itemize}
Face à ces limitations temporelles et à l'échéance de la soutenance, la chaîne numérique est restée figée dans sa configuration de validation de bas niveau (boucle ouverte).

\subsubsection{Code source final embarqué (Validation de la chaîne PWM)}
\begin{lstlisting}[language=C, caption={fichier main.c en BO}, label={code:main_final}]
/* USER CODE BEGIN Header */
/**
  *****************************************************************
  * @file           : main.c
  * @brief          : Body du programme de validation de la chaine PWM
  *****************************************************************
  */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // 1. Demarre le signal PWM physique sur la broche PA8
  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1);

  // 2. Active la sortie principale 
  ...
  while (1)
  {
    //rapport cyclique de 0 a 255
    for (uint16_t dc = 0; dc <= 255; dc++)
    {
        __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, dc);
        HAL_Delay(5); // Transition visible a l'oscilloscope
    }

    //rapport cyclique de 255 a 0
    for (uint16_t dc = 255; dc > 0; dc--)
    {
        __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, dc);
        HAL_Delay(5);
    }
...
static void MX_TIM1_Init(void)
{
  ...
  htim1.Init.Period = 255; // Resolution fine demandee
