...
Les mesures ont été effectuées sur la maquette en chargeant le système avec la bobine. Les signaux ont été capturés à l'oscilloscope.

\subsection{Régime transitoire et respect du cahier des charges}
L'analyse de la réponse à un échelon (Relevés \textit{Newfile1.png} et \textit{Newfile2.png}) met en évidence les performances temporelles suivantes :
\begin{itemize}
    \item \textbf{Dépassement ($D\,\%$)} : Le pic de courant atteint précisément la valeur limite fixée par le cahier des charges, soit un dépassement de \qty{5}{\%} ($1,05\text{ V}$ pour une consigne de $1\text{ V}$). Ce résultat valide parfaitement la robustesse de notre réglage numérique incluant le verrou logique de l'intégrateur.
    \item \textbf{Constante de temps ($\tau$)} : Elle est mesurée graphiquement à \qty{63}{\%} de la valeur finale, ce qui donne $\tau = \dots\dots\text{ ms}$ (à compléter d'après vos curseurs).
    \item \textbf{Temps de réponse à \qty{95}{\%} ($t_{r5\%}$)} : Le courant entre durablement dans la bande des \qty{5}{\%} au bout de $t_{r5\%} = \dots\dots\text{ ms}$ (à compléter d'après vos curseurs).
\end{itemize}

\subsection{Régime permanent et erreur statique}
Le suivi des signaux en régime permanent (\textit{Newfile3.png}) montre l'évolution de la commande et du retour de courant. L'action intégrale remplit parfaitement son rôle :
\begin{itemize}
    \item \textbf{Erreur statique ($\varepsilon_S$)} : L'erreur en régime permanent est mesurée à $\varepsilon_S = \dots\dots\text{ V}$ (à compléter, théoriquement nulle ou de l'ordre de quelques mV liés au bruit de quantification). Le courant rejoint précisément la valeur de consigne imposée.
\end{itemize}
